...
/**
 * Apple touch icon (180x180) — used by iOS when adding the site to the
 * home screen. Composited from the school's logo on a teal background.
 * Falls back to /icons/apple-touch-icon.png (read directly) if no logo
 * is set or any error occurs.
 */
...
export async function GET() {
  try {
    ...
    console.error('[api/pwa/apple-touch-icon] error:', err);
    return streamFallback();
  }
}
